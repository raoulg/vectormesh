...
logger.add("logs/embed_deberta.log", rotation="10 MB", level="DEBUG")


def main():
    """
    microsoft/deberta-v3-small:
        DeBERTa v3 - DeBERTa-v3-large outperforms on five out of nine tasks in the
        Dutch Model Benchmark Clinjournal, even though it's technically English.
        DeBERTa uses more sophisticated positional encoding and attention mechanisms
        than BERT/RoBERTa. There's also mDeBERTa (multilingual DeBERTa) which includes Dutch.
    """
    assets = Path("assets")
    ...
    vectorizer = Vectorizer(model_name=model_name, col_name="debertav3-small")
